...
# Used to separate multipart, You can set your own
...
function banner($info)
{
    ...
    $bbox = imagettfbbox($font_size, 0, $font, 'No file');
    $x = $bbox[0] + (imagesx($im) / 2) - ($bbox[4] / 2) - 25;
    $y = $bbox[1] + (imagesy($im) / 2) - ($bbox[5] / 2) - 5;
    imagettftext($im, $font_size, 0, $x, $y - $font_size / 2, $text_color, $font, 'No Image');
        ...
        if ($n == 0) {
            if (!file_exists($file)) {
                ...
                if ($i % 2) {
                    ...
                    $bbox = imagettfbbox($font_size, 0, $font, 'No Updates');
                    $x = $bbox[0] + (imagesx($im) / 2) - ($bbox[4] / 2) - 25;
                    $y = $bbox[1] + (imagesy($im) / 2) - ($bbox[5] / 2) - 5;
                    imagettftext($im, $font_size, 0, $x, $y - $font_size * 0.5, $text_color, $font, 'No updates');
                    $bbox = imagettfbbox($font_size * 0.33, 0, $font, "Last frame received:");
                    $x = $bbox[0] + (imagesx($im) / 2) - ($bbox[4] / 2) - 25;
                    $y = $bbox[1] + (imagesy($im) / 2) - ($bbox[5] / 2) - 5;
                    imagettftext($im, $font_size*0.33, 0, $x, $y + $font_size * 0.5, $text_color, $font, "Last frame received:");
